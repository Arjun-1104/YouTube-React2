import React, { useEffect, useState } from "react";

import { IoIosArrowDown } from "react-icons/io";
import { LiaDownloadSolid } from "react-icons/lia";
import { RiCloseLargeFill } from "react-icons/ri";
import { HiDotsVertical } from "react-icons/hi";
import { SlHeart } from "react-icons/sl";
import { LiaSmileBeam } from "react-icons/lia";
import { RiMoneyDollarBoxLine } from "react-icons/ri";
import { VscSend } from "react-icons/vsc";

import { generateRandomName, generateRandomMessage } from "./Helper";
import { useSelector } from "react-redux";
import Picker from 'emoji-picker-react'

const LiveChat = () => {
  const [chat, setChat] = useState([]);
  const [liveChatInput, setLiveChatInput] = useState("");
  const [showPicker, setShowPicker] = useState(false);
  const theme = useSelector((store) => store.app.theme);

  useEffect(() => {
    let timeout = setInterval(() => {
      setChat([
        {
          name: generateRandomName(),
          message: generateRandomMessage(16),
        },
        ...chat,
      ]);
    }, 2000);

    return () => clearInterval(timeout);
    // },[])
  }, [chat]);

  const onEmojiClick = (event ,emojiObj) => {
    setLiveChatInput(prev => prev + emojiObj.emoji)
    setShowPicker(false)
  }

  const handleLiveChatInput = (value) => {
    setLiveChatInput(value);
  };

  const handleLiveChatSend = () => {
    setChat([
      {
        name: "Arjun",
        message: liveChatInput,
      },
      ...chat,
    ]);

    setLiveChatInput("");
  };

  return (
    <div
      className={`${
        theme ? "border-white/20" : "border-gray-200"
      } w-[30%] h-[85vh] ms-4 rounded-xl relative border-2 `}
    >

      <div className="py-1 px-2 flex items-center justify-between h-[12%]">
        <div className="flex items-center gap-1 px-2 cursor-pointer">
          <h1 className="text-medium">Top chat</h1>
          <IoIosArrowDown size={"20px"} />
        </div>
        <div className="flex">
          <div
            className={`${
              theme ? "hover:bg-white/25" : "hover:bg-gray-200"
            } rounded-full p-2 cursor-pointer`}
          >
            <HiDotsVertical size={"24px"} />
          </div>
          <div
            className={`${
              theme ? "hover:bg-white/25" : "hover:bg-gray-200 "
            } rounded-full p-2 cursor-pointer`}
          >
            <RiCloseLargeFill size={"24px"} />
          </div>
        </div>
      </div>

      <div className="pe-4 h-[76%] overflow-y-auto flex flex-col-reverse">
        {chat.map(({ name, message }, idx) => {
          return (
            <div
              key={idx}
              className={`${
                theme ? "hover:bg-white/20" : "hover:bg-gray-100"
              } flex items-center gap-4 ps-4 py-1 cursor-pointer`}
            >
              <img
                className="cursor-pointer"
                width={"24px"}
                src="data:image/png;base64,"
              ></img>
              <div className="flex gap-2">
                <span className="text-gray-400 font-medium text-sm">
                  {name}
                </span>
                <p className="text-xs">{message}</p>
              </div>
            </div>
          );
        })}
      </div>
      
      
      <div
        className={`${
          theme ? "border-white/20" : "border-gray-200"
        } bottom-0 w-full p-2 h-[12%] border-t-2 relative`}
      >
         <div className=" flex items-center justify-around w-[100%]">
        <div
          className={`${
            theme ? "bg-white/20 font-semibold" : "bg-gray-100"
          } w-[70%] flex rounded-full `}
        >
          <input
            type="text"
            className="w-[85%] px-3 text-sm outline-0"
            placeholder="Chat..."
            onChange={(e) => handleLiveChatInput(e.target.value)}
            value={liveChatInput}
          />
          <div className="rounded-full p-2 cursor-pointer" onClick={()=> setShowPicker(val => !val)}>
            <LiaSmileBeam size={"22px"} />
          </div>
        </div>
        
        {liveChatInput.length === 0 && (
          <div className="flex items-center w-[24%] justify-between">
            <div
              className={`${
                theme
                  ? "bg-white/20 hover:bg-white/25 text-white"
                  : "bg-gray-100"
              } rounded-full p-2 cursor-pointer`}
            >
              <RiMoneyDollarBoxLine size={"20px"} />
            </div>

            <div
              className={`${
                theme
                  ? "bg-white/20 hover:bg-white/25 text-white"
                  : "bg-gray-100"
              } rounded-full p-2 cursor-pointer`}
            >
              <SlHeart size={"20px"} />
            </div>
          </div>
        )}
        
        {liveChatInput && (
          <div
            className={`${
              theme ? "bg-white/20 hover:bg-white/25 text-white" : "bg-gray-100"
            } rounded-full p-2 cursor-pointer`}
            onClick={() => handleLiveChatSend()}
          >
            <VscSend size={"20px"} />
          </div>
        )}
       </div>
        <div>
          {showPicker && <Picker className="fixed" onEmojiClick={onEmojiClick} />}
        </div>
      </div>
     
    </div>
  );
};

export default LiveChat;
