import React, { useEffect, useState } from "react";

import { IoIosArrowDown } from "react-icons/io";
import { LiaDownloadSolid } from "react-icons/lia";
import { RiCloseLargeFill } from "react-icons/ri";
import { HiDotsVertical } from "react-icons/hi";
import { SlHeart } from "react-icons/sl";
import { LiaSmileBeam } from "react-icons/lia";
import { RiMoneyDollarBoxLine } from "react-icons/ri";

import {generateRandomName,generateRandomMessage} from './Helper';



const LiveChat = () => {
    const [chat ,setChat] = useState([]);
    const [liveChatInput, setLiveChatInput] = useState('');

    useEffect(()=> {
        let timeout = setInterval(()=> {
            setChat([{
                name: generateRandomName(),
                message: generateRandomMessage(16)
            },...chat])
        },2000)

        return ()=> clearInterval(timeout)
    // },[])
    },[chat])

    const handleLiveChatInput = (value) => {
        setLiveChatInput(value)  
    }

    const handleLiveChatSend = () => {
      setChat([{
        name: 'Arjun',
        message: liveChatInput
      },...chat])

      setLiveChatInput('')

    }

  return (
    <div className=" w-[30%] h-[85vh] ms-4 rounded-xl relative border-2 border-gray-200 ">
      <div className=" py-1 px-2 flex items-center justify-between h-[12%]">
        <div className="flex items-center gap-1 px-2 cursor-pointer">
          <h1 className="text-medium">Top chat</h1>
          <IoIosArrowDown size={"20px"} />
        </div>
        <div className="flex ">
          <div className="hover:bg-gray-200 rounded-full p-2 cursor-pointer">
            <HiDotsVertical size={"24px"} />
          </div>
          <div className="hover:bg-gray-200 rounded-full p-2 cursor-pointer">
            <RiCloseLargeFill size={"24px"} />
          </div>
        </div>
      </div>


      <div className="pe-4 h-[76%] overflow-y-auto flex flex-col-reverse">
        {
            chat.map(({name,message},idx)=>{
                return <div key={idx} className="flex items-center gap-4 ps-4 py-1 hover:bg-gray-100 cursor-pointer ">
                <img
                    className="cursor-pointer"
                    width={"24px"}
                    src="data:image/png;base64,"
                  ></img>
                <div className="flex gap-2">
                    <span className="text-gray-400 font-medium">{name}</span>
                    <p className="text-sm">{message}</p>
                </div>
                </div>
            })
        }
      </div>

      <div className="absolute bottom-0 flex items-center justify-around w-full p-2 h-[12%] border-t-2  border-gray-200">
        <div className="bg-gray-100 w-[70%] flex rounded-full">
          <input
            type="text"
            className="w-[85%] px-3 text-sm outline-0"
            placeholder="Chat..."
            onChange={(e)=> handleLiveChatInput(e.target.value)}
            value={liveChatInput}
            
          />
          {console.log(liveChatInput)}
          <div className=" rounded-full p-2 cursor-pointer">
            <LiaSmileBeam size={"22px"} />
          </div>
        </div>

        {liveChatInput.length===0 && <div className="flex items-center w-[24%] justify-between">
          <div className="bg-gray-100 rounded-full p-2 cursor-pointer">
            <RiMoneyDollarBoxLine size={"20px"} />
          </div>

          <div className="bg-gray-100 rounded-full p-2 cursor-pointer">
            <SlHeart size={"20px"} />
          </div>
        </div>}

        {liveChatInput && <div className="bg-gray-100 rounded-full p-2 cursor-pointer" onClick={()=> handleLiveChatSend()}>
            <LiaDownloadSolid size={"20px"} />
          </div>}
      </div>
      
    </div>
  );
};

export default LiveChat;
